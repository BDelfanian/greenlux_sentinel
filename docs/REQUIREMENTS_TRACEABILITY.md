...
| 10 | Real-world finance/investment trend, Luxembourg preferred | Greenwashing-risk / SFDR-consistency, tied directly to CSSF's stated 2026 supervisory priorities — see [DATA.md](DATA.md#why-this-topic) |
| 11 | GitHub repo, regular pushes | [github.com/BDelfanian/greenlux_sentinel](https://github.com/BDelfanian/greenlux_sentinel) |
| 12 | Real moderate-size Kaggle dataset; relational + NoSQL with different file formats | Morningstar Funds (CSV → Postgres) + ETF holdings/company ESG (CSV/JSON reshaped → Cosmos DB) — see [DATA.md](DATA.md#datasets) |
...
| 14 | At least one API for dynamic external data | GLEIF API — live Luxembourg legal-entity lookups |
